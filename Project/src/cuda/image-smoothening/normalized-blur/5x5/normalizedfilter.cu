...
__global__ void apply_filter(unsigned char* input, unsigned char* output, int height, int width) {
    ...
    r_out += image[NUM_CHANNELS*((im_x-1)*smem_y+im_y+1)];
    r_out += image[NUM_CHANNELS*((im_x+1)*smem_y+im_y+1)];
    r_out += image[NUM_CHANNELS*(im_x*smem_y+im_y-2)];
    r_out += image[NUM_CHANNELS*(im_x*smem_y+im_y+2)];
    r_out += image[NUM_CHANNELS*((im_x-2)*smem_y+im_y)];
    r_out += image[NUM_CHANNELS*((im_x+2)*smem_y+im_y)];
    r_out += image[NUM_CHANNELS*((im_x-2)*smem_y+im_y-2)];
    r_out += image[NUM_CHANNELS*((im_x+2)*smem_y+im_y-2)];
    r_out += image[NUM_CHANNELS*((im_x-2)*smem_y+im_y+2)];
    r_out += image[NUM_CHANNELS*((im_x+2)*smem_y+im_y+2)];
    r_out += image[NUM_CHANNELS*((im_x-2)*smem_y+im_y-1)];
    r_out += image[NUM_CHANNELS*((im_x-1)*smem_y+im_y-2)];
    r_out += image[NUM_CHANNELS*((im_x-2)*smem_y+im_y+1)];
    r_out += image[NUM_CHANNELS*((im_x-1)*smem_y+im_y+2)];
    r_out += image[NUM_CHANNELS*((im_x+2)*smem_y+im_y-1)];
    r_out += image[NUM_CHANNELS*((im_x+1)*smem_y+im_y-2)];
    ...
    g_out += image[NUM_CHANNELS*((im_x-1)*smem_y+im_y+1)+1];
    g_out += image[NUM_CHANNELS*((im_x+1)*smem_y+im_y+1)+1];
    g_out += image[NUM_CHANNELS*(im_x*smem_y+im_y-2)+1];
    g_out += image[NUM_CHANNELS*(im_x*smem_y+im_y+2)+1];
    g_out += image[NUM_CHANNELS*((im_x-2)*smem_y+im_y)+1];
    g_out += image[NUM_CHANNELS*((im_x+2)*smem_y+im_y)+1];
    g_out += image[NUM_CHANNELS*((im_x-2)*smem_y+im_y-2)+1];
    g_out += image[NUM_CHANNELS*((im_x+2)*smem_y+im_y-2)+1];
    g_out += image[NUM_CHANNELS*((im_x-2)*smem_y+im_y+2)+1];
    g_out += image[NUM_CHANNELS*((im_x+2)*smem_y+im_y+2)+1];
    g_out += image[NUM_CHANNELS*((im_x-2)*smem_y+im_y-1)+1];
    g_out += image[NUM_CHANNELS*((im_x-1)*smem_y+im_y-2)+1];
    g_out += image[NUM_CHANNELS*((im_x-2)*smem_y+im_y+1)+1];
    g_out += image[NUM_CHANNELS*((im_x-1)*smem_y+im_y+2)+1];
    g_out += image[NUM_CHANNELS*((im_x+2)*smem_y+im_y-1)+1];
    g_out += image[NUM_CHANNELS*((im_x+1)*smem_y+im_y-2)+1];
    ...
    b_out += image[NUM_CHANNELS*((im_x-1)*smem_y+im_y+1)+2];
    b_out += image[NUM_CHANNELS*((im_x+1)*smem_y+im_y+1)+2];
    b_out += image[NUM_CHANNELS*(im_x*smem_y+im_y-2)+2];
    b_out += image[NUM_CHANNELS*(im_x*smem_y+im_y+2)+2];
    b_out += image[NUM_CHANNELS*((im_x-2)*smem_y+im_y)+2];
    b_out += image[NUM_CHANNELS*((im_x+2)*smem_y+im_y)+2];
    b_out += image[NUM_CHANNELS*((im_x-2)*smem_y+im_y-2)+2];
    b_out += image[NUM_CHANNELS*((im_x+2)*smem_y+im_y-2)+2];
    b_out += image[NUM_CHANNELS*((im_x-2)*smem_y+im_y+2)+2];
    b_out += image[NUM_CHANNELS*((im_x+2)*smem_y+im_y+2)+2];
    b_out += image[NUM_CHANNELS*((im_x-2)*smem_y+im_y-1)+2];
    b_out += image[NUM_CHANNELS*((im_x-1)*smem_y+im_y-2)+2];
    b_out += image[NUM_CHANNELS*((im_x-2)*smem_y+im_y+1)+2];
    b_out += image[NUM_CHANNELS*((im_x-1)*smem_y+im_y+2)+2];
    b_out += image[NUM_CHANNELS*((im_x+2)*smem_y+im_y-1)+2];
    b_out += image[NUM_CHANNELS*((im_x+1)*smem_y+im_y-2)+2];
