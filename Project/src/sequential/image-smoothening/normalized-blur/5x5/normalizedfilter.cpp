...
void apply_filter(unsigned char* image, unsigned char* output, int height, int width, int x, int y) {
    ...
    if((x>=1) && (y>=2)) { //INDEX 1
        r_out+=((float)image[NUM_CHANNELS*((x-1)*height+y-2)])/255;
        g_out+=((float)image[NUM_CHANNELS*((x-1)*height+y-2)+1])/255;
        b_out+=((float)image[NUM_CHANNELS*((x-1)*height+y-2)+2])/255;
    } 

    if((y>=2)) { //INDEX 2
        r_out+=((float)image[NUM_CHANNELS*(x*height+y-2)])/255;
        g_out+=((float)image[NUM_CHANNELS*(x*height+y-2)+1])/255;
        b_out+=((float)image[NUM_CHANNELS*(x*height+y-2)+2])/255;
    }
    ...
    if((x>=2) && (y>=1)) { //INDEX 5
        r_out+=((float)image[NUM_CHANNELS*((x-2)*height+y-1)])/255;
        g_out+=((float)image[NUM_CHANNELS*((x-2)*height+y-1)+1])/255;
        b_out+=((float)image[NUM_CHANNELS*((x-2)*height+y-1)+2])/255;
    } 
    ...
    if((x>=2)) { //INDEX 10
        r_out+=((float)image[NUM_CHANNELS*((x-2)*height+y)])/255;
        g_out+=((float)image[NUM_CHANNELS*((x-2)*height+y)+1])/255;
        b_out+=((float)image[NUM_CHANNELS*((x-2)*height+y)+2])/255;
    } 
