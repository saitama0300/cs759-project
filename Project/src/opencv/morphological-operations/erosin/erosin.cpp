...
int main( int argc, char** argv )
{
  ...
  if( src.empty() )
  {
   ...
   Mat element = getStructuringElement( MORPH_RECT, Size(KERNEL_SIZE, KERNEL_SIZE), Point( KERNEL_SIZE/2, KERNEL_SIZE/2 ) );
    
   duration<double, std::milli> duration_sec;
   start = high_resolution_clock::now();
   erode( src, erosin_dst, element );
   end = high_resolution_clock::now();
   duration_sec = std::chrono::duration_cast<duration<double, std::milli>>(end - start);
   cout <<"Runtime: "<<duration_sec.count() << "\n";

   imwrite(parser.get<String>( "@output" ),erosin_dst);
