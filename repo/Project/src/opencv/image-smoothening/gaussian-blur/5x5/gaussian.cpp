...
int main( int argc, char** argv )
{
  ...
  GaussianBlur( src, gaussian_dst, Size(KERNEL_SIZE,KERNEL_SIZE), 0, 0 ); //0,0 here refers to gaussian std deviation which is 0 ideally for both x and y direction
  end = high_resolution_clock::now();

  duration_sec = std::chrono::duration_cast<duration<double, std::milli>>(end - start);
  cout <<"Runtime: "<<duration_sec.count() << "\n";

  imwrite(parser.get<String>( "@output" ),gaussian_dst);
